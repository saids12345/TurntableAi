...
function inferStatus(row: ReviewRow): ReviewStatus {
...
function normalizePlatform(value: string | null): Platform {
  ...
  return lower.includes("yelp") ? "Yelp" : "Google";
}
  ...
  try {
    ...
    const [
      ...
      supabase
        .from("reviews")
        .select(
          "id, location_name, reviewer_name, rating, text, update_time, create_time, platform, owner_reply"
        )
        .eq("user_id", user.id)
    ...
    if (reviewsError) {
      console.error("operator-snapshot/reviews reviews error:", reviewsError);
      return NextResponse.json({ error: "Failed to load reviews" }, { status: 500 });
    }

    if (workflowError) {
      console.error("operator-snapshot/reviews workflow error:", workflowError);
    ...
    const reviews: SnapshotReview[] = ((reviewsData ?? []) as ReviewRow[])
      ...
      .map((row) => {
        ...
        const derivedStatus = workflow?.status ?? inferStatus(row);
        const fallbackSuggestedReply = buildSuggestedReply({
          reviewerName,
          issueType,
        });
        const createdAt =
          row.update_time || row.create_time || new Date().toISOString();
        ...
        return {
          ...
          issueType,
          suggestedReply:
            workflow?.suggested_reply?.trim() ||
            row.owner_reply?.trim() ||
            fallbackSuggestedReply,
          note: workflow?.note ?? "",
    ...
    console.error("operator-snapshot/reviews unexpected error:", error);
